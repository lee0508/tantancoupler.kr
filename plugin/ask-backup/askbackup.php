...
<div class="local_desc02 local_desc">
    ...
    <p>백업완료되면 자동다운로드 됩니다. 대용량 DB는 shell에서 직접 Dump 하시기 바랍니다.</p>
    <p>테이블에 부담이 있으면 최적화 버튼이 출력됩니다. 해당 테이블 선택 후 실행하세요. 테이블 용량이 크고 접속이 많은 시간대는 피하시기 바랍니다.</p>
</div>
...
<section class="db-list">
    <form name="fbackup" id="fbackup" action="./askbackup.update.php" method="post" enctype="multipart/form-data">
        ...
        $table .= "<tr>";
        $table .= "<th class='table-no'><label><input type='checkbox' name='allcheck' value=''> All</label></th><th>테이블명</th><th>데이터정렬방식</th><th>엔진</th><th>행</th><th>크기(Kb)</th><th>부담</th>";
        $table .= "</tr>";
        $table .= "<tbody>";
        $overhead_size = 0;
